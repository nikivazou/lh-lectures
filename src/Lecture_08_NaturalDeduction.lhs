...
In a previous lecture we saw that programs are proofs and we 
...
In Liquid Haskell, it is encoded using the below type specification:
...
**Existential quantification** is encoded by the dependent pair type.
It has two rules: _introduction_ and _elimination_:
